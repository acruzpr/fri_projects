...
class BasinHopping(Dynamics):
    ...
    def run(self, steps):
        ...
        for step in range(steps):
            ...
            self.record_norm_dists(rn)
            if En < self.Emin:
                self.Emin = En
                self.rmin = self.atoms.get_positions()
                self.call_observers()
                tsase.io.write_con('best_energies.con',self.atoms,w='a')
...
### EDIT CODE FOR PROBLEM 5 HERE ###########################################
            ...
            disp = self.dr * np.random.random_integers(-1,1,size=(len(atoms), 3))
############################################################################
        else:
            ...
            for i in range(len(positions)):
                for j in range(i+1,len(positions)):
                    ...
                    if magd < self.pushapart and magd > 0:
                        moved += 1
                        vec = d/magd
                        #vec = d/magd/alpha * (self.pushapart - magd) * .7
                        movea[i] += alpha *vec
                        movea[j] -= alpha *vec
            positions += movea
            total += 1
    ...
    def record_norm_dists(self,positions):
        vals = [0] * len(positions)
        for i in range(len(vals)):
            d = positions[i] - self.cm
            vals[i] = np.sqrt(np.vdot(d,d))
        list.sort(vals)
        for i in range(len(vals)):
            self.norm_dists[i].append(vals[i])
    ...
    def get_norm_dists(self):
        return self.norm_dists()
